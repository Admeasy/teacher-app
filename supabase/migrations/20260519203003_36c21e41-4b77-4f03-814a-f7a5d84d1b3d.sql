...
-- Add activity configs to timetable_settings
...
-- Add slash columns to timetable for optional pairings
...
-- Per-class subject catalogue (used by the Subjects tab on the class page)
CREATE TABLE IF NOT EXISTS public.class_subjects (
  ...
  kind text NOT NULL DEFAULT 'major', -- 'major' | 'optional' | 'activity'
  is_major boolean NOT NULL DEFAULT true,
  optional_group text, -- shared label for paired optionals (e.g. 'Group A')
...
CREATE POLICY "ws read class_subjects"   ON public.class_subjects FOR SELECT USING (is_workspace_member(workspace_id));
CREATE POLICY "ws insert class_subjects" ON public.class_subjects FOR INSERT WITH CHECK (is_workspace_member(workspace_id));
CREATE POLICY "ws update class_subjects" ON public.class_subjects FOR UPDATE USING (is_workspace_member(workspace_id));
CREATE POLICY "ws delete class_subjects" ON public.class_subjects FOR DELETE USING (is_workspace_member(workspace_id));
